...
RSpec.describe Bragi::ItemSerializer do
  ...
  it 'has an assumed set of attributes' do
    expect(serialization[:data][:attributes].keys).to eq %i[after_id audio_url audio_identifier audio_title audio_description audio_hosts audio_program audio_publish_datetime audio_image_url origin_url source playtime status finished]
  end
end
